...
- **`WidgetBase` 는 Canvas 하위 UI 오브젝트에만 붙입니다.** UI 컴포넌트라면
  ...
  Widget(을 비롯한 UI 클래스 전반)에는 과도한 기능을 넣지 않습니다. 검증이나 상태
  전이까지 스스로 떠맡기지 말고, `Action`/`event`로 노출해서 Director나 Manager가
  구독·위임하는 방식으로 구현하세요 — Widget은 표시와 입력 전달만 책임집니다.
- **씬을 스코프로 하는 매니저 성격의 클래스는 `DirectorBase`(프로젝트에서 확장한
  하위 클래스 포함)를 상속**하세요. 씬이 언로드되면 자연히 정리되고,
  `DirectorFacade.GetDirector<T>()`로 일관되게 조회할 수 있습니다. 씬 스코프인데
  싱글턴이나 `...Manager`로 만드는 것이 가장 흔한 오용입니다.
- **싱글턴은 지양합니다.** 씬을 넘어 게임 전체를 스코프로 하는 진짜 매니저에만
  씁니다 — 그런 경우에만 **`Singleton<T>`** / **`SingletonAsset<T>`** 을 쓰고
  클래스명에 `...Manager`를 붙이며, `PrefabManager` 의 `initialSpawnPrefabs` 에
  등록해 DontDestroyOnLoad 영역에서 부팅되게 하세요. 그 외(씬 스코프)에는 위의
  Director 규칙을 따릅니다. 직접 싱글턴을 구현하지 마세요.
...
이 라이브러리의 패턴(Director/Manager/Widget)을 쓸 때도, 호스트 프로젝트 코드를
쓸 때도 따라야 하는 일반 원칙입니다.

- **역참조 금지.** 더 높은 권한/범위의 클래스(Director, Manager)를 더 낮은 권한의
  클래스(Controller, Widget, Handler)가 직접 참조·호출하지 않습니다. 의존 방향은
  항상 위 → 아래입니다. 낮은 쪽은 `event`/`Action`을 노출하고, 높은 쪽이 그걸
  구독해서 검증·적용을 책임집니다. 거꾸로 참조하면 상위 쪽을 교체하거나
  리팩터링할 때마다 하위 클래스까지 줄줄이 고쳐야 합니다.
- **필수 의존성에는 널 체크를 하지 않습니다.** `[RequireComponent]`로 보장되거나
  초기화 순서상 항상 존재해야 하는 참조를 매번 `!= null`로 감싸면, 배선이 실제로
  잘못됐을 때 원인이 늦게, 혹은 엉뚱한 곳에서 드러납니다. 즉시 NRE로 터지게 두고
  바로 원인을 찾으세요. 널 체크는 정말 선택적인 참조이거나 파괴 시점의 경쟁
  상태처럼 있을 수 있는 경우에만 씁니다.
- **인터페이스는 기능·역할 기준으로 최대한 추상적으로 이름 짓습니다.** 특정 클래스
  계층이 아니라 "이 능력을 가진 모든 것"을 가리키도록 설계하세요 — 예: 데미지를
  받을 수 있는 모든 대상은 `IDamagable`, 상호작용 가능한 모든 대상은
  `IInteractable`. 여러 시스템에서 자유롭게 구현하고 호출할 수 있다는 전제로
  설계합니다.
- **임시 코드는 임시라고 명시합니다** (`// TODO`, `// TEMP` 등). 반대로 전체
  구조·아키텍처 자체를 "일단 임시로" 설계하지는 않습니다 — 구조는 처음부터
  제대로, 임시일 수 있는 건 세부 구현뿐입니다.
- **네이밍은 기존 맥락을 따릅니다.** 새 클래스/함수를 추가할 때도 그 프로젝트에서
  이미 쓰고 있는 네이밍 규칙(접두사, 역할별 접미사 등)을 그대로 따르세요. 이름의
  의도가 모호하면 안 됩니다 — 예를 들어 `Controller`는 플레이어 조작이 들어가는
  뉘앙스이므로, 무언가를 관리·조율만 하는 컴포넌트라면 `Handler`가 더 정확합니다.
  애매하면 관용적으로 굳어진 이름 하나로 단정하지 말고 실제 역할을 기준으로
  판단하세요.
